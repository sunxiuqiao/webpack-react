...
function Top(){
    const top_items =['豆瓣', '读书','电影','音乐', '同城', '小组', '阅读', 'FM', '时间', '市集', '更多']
    return(
        <div style={style.top}>
            <div style={style.top1}>
                <div>
                    <ul>
                        {
                            top_items.map(function(item,i){
                                return<li key={i} style={style.top_item}><a href='' style={style.top_item_a}>{item}</a></li>
                            })
                        }
                    </ul>
                </div>
